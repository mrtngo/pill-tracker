...
async function run() {
  ...
  const nonStarlinkLogs = [];
  for (const log of data) {
    const starlink = await isStarlinkIp(log.ip, cache);
    if (!starlink) {
      nonStarlinkLogs.push(log);
      if (nonStarlinkLogs.length === 10) {
        break;
      }
    }
  }

  if (nonStarlinkLogs.length === 0) {
    console.log("\nNo non-Starlink accesses found in the logs.");
    return;
  }

  console.log(`\nLast ${nonStarlinkLogs.length} non-Starlink accesses:`);
  nonStarlinkLogs.forEach((log, index) => {
    ...
    console.log("  Device/Client ID:", log.device_id || log.client_id || "null");
  });
}
